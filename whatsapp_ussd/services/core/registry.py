...
#         """Auto-discover and register all handlers dynamically"""
#         package = 'whatsapp_ussd.services.states'
#         for _, module_name, _ in pkgutil.iter_modules([str(Path(__file__).parent.parent / "states")]):
#             module = importlib.import_module(f"{package}.{module_name}")
#             for attr in dir(module):
#                 obj = getattr(module, attr)
#                 if isinstance(obj, type) and issubclass(obj, BaseStateHandler) and obj is not BaseStateHandler:
#                     cls.register(obj.state_name, obj)
#         cls.validate_graph()
    
#     @classmethod
#     def validate_graph(cls):
#         """Ensure all next_state transitions point to registered states."""
#         unknown_transitions = []            
#         for handler_class in cls._handlers.values():
#             allowed = handler_class(None).get_allowed_transitions()
#             for state in allowed:
#                 if state not in cls._handlers:
#                     unknown_transitions.append((handler_class.state_name, state))

#         if unknown_transitions:
#             raise ValueError(
#                 f"Invalid transitions found: {unknown_transitions}"
#             )
